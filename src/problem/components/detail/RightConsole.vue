...
import {
  getSubmissionById,
  submit,
} from "@/problem/model/domainLogic/submission";
...
const currentSubmission = computed(
  () => store.state.problemStore.submission
) as Ref<SubmissionModel>;
const currentProblemCode = computed(
  () => store.state.problemStore.currentProblemCode
) as Ref<ProblemCode>;
const editorSettings = computed(
  () => store.state.problemStore.editorSettings
) as Ref<EditorSetting>;
...
async function handleSubmit() {
  try {
    ...
    let submissionId = await submit(
      problem.value,
      currentProblemCode.value.codes[editorSettings.value.language],
      editorSettings.value.language
    );
    showSubmissionTab();
