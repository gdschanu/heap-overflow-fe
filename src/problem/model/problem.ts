...
class Problem {
  ...
  addMemoryLimit(newMemoryLimit: MemoryLimit): void {
    for (const memoryLimit of this.memoryLimits) {
      if (
        memoryLimit.getProgrammingLanguage() ===
        newMemoryLimit.getProgrammingLanguage()
      ) {
        throw new Error(
          "Memory limit for this programming language already exists"
        );
      }
    }
  ...
  addTimeLimit(newTimeLimit: TimeLimit): void {
    for (const timeLimit of this.timeLimits) {
      if (
        timeLimit.getProgrammingLanguage() ===
        newTimeLimit.getProgrammingLanguage()
      ) {
        throw new Error(
          "Time limit for this programming language already exists"
        );
      }
    }
  ...
  addAllowedProgrammingLanguage(
    newAllowedProgrammingLanguage: ProgrammingLanguage
  ): void {
    for (const allowedProgrammingLanguage of this.allowedProgrammingLanguages) {
      if (allowedProgrammingLanguage === newAllowedProgrammingLanguage) {
        throw new Error(
          "duplicate programming language: " + newAllowedProgrammingLanguage
        );
      }
    }
