<template>
    <button
        :title="des"
        class="transition block whitespace-nowrap relative rounded-lg px-5 py-2.5 font-bold"
        :class="`${type}${disabled ? '-disabled' : ''} ${block ? blockButton : ''}`"
        :disabled="disabled"
        @click="clicked"
    >
...
const blockButton = computed(() => 'w-full')
...
.primary {
    ...
    dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-100 dark:active:bg-slate-200 dark:focus:ring-slate-300;
}
.primary-disabled {
